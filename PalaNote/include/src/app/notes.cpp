...
void loadIndex() {
  ...
  while (f.available()) {
    ...
    e.tag[31]='\0';
    e.uploaded = (ln.substring(c2+1).toInt()==1);
...
void saveIndex() {
  ...
  for (int i=0; i<(int)noteIndex.size(); i++)
    f.printf("%d,%s,%d\n", noteIndex[i].num, noteIndex[i].tag, noteIndex[i].uploaded?1:0);
  f.close();
  if (SD_MMC.exists(INDEX_FILE)) SD_MMC.remove(INDEX_FILE);
  SD_MMC.rename(tmp, INDEX_FILE);
}

void addToIndex(int num, const char* tag, bool uploaded) {
  NoteEntry e;
  e.num = num;
...
// Marks a note as dealt with after it was uploaded to the notes server.
// There is usually no local .txt: the transcript lives on the server, so
// this only stops the note being uploaded again and records the fact in
// the meta file. The OpenAI fallback path calls this too: it has
// already written a local .txt, and `uploaded` only tracks whether the note
// still needs sending - noteHasLocalText() is what answers "is there text?".
void markNoteUploaded(int num) {
  const char* foundTag = "";
  for (int i = 0; i < (int)noteIndex.size(); i++) {
...
void writeNoteMeta(int num, const char* tag) {
  ...
  // Preserved across rewrites: the server's id for this note is what lets a
  // later sync fetch its processed markdown back.
  String meetingId = readNoteMetaValue(num, "meeting_id");
  File f = SD_MMC.open(path.c_str(), FILE_WRITE);
  if (!f) return;
  f.print("created_utc="); f.println(created);
  f.print("tag="); f.println(tag ? tag : "");
  if (meetingId.length()) { f.print("meeting_id="); f.println(meetingId); }
  f.print("synced=");
  bool uploaded = false;
  for (int i = 0; i < (int)noteIndex.size(); i++)
    if (noteIndex[i].num == num) { uploaded = noteIndex[i].uploaded; break; }
  f.println(uploaded ? "1" : "0");
  f.close();
}

void setNoteMeetingId(int num, long meetingId) {
  ...
  f.print("meeting_id="); f.println(meetingId);
  f.print("synced="); f.println(synced.length() ? synced : "0");
  f.close();
}

long noteMeetingId(int num) {
  String v = readNoteMetaValue(num, "meeting_id");
  return v.length() ? v.toInt() : 0;
}

// True once the processed markdown from the server is on the card.
bool noteHasLocalText(int num) {
  char p[64];
  snprintf(p, sizeof(p), "%s/note_%03d.txt", NOTES_DIR, num);
...
String noteTickerText(int idx) {
  ...
  if (preview.length() == 0)
    preview = noteIndex[idx].uploaded ? "processing..." : "not synced";
  String ticker = dt;
...
int sdUsedPercent() {
  ...
  if (total == 0) return -1;
  return (int)((SD_MMC.usedBytes() * 100ULL) / total);
}
